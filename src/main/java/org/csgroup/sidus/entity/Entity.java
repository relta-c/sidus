...
import net.chifumi.stellar.graphic.Sprite;
...
public class Entity extends Sprite {
    ...
    public Entity(final SpriteMap spriteMap, final CharSequence name) {
    ...
    public void moveToAngle(final float angle, final float distant) {
        final float newX = (float) (getOrigin().getX() + StrictMath.cos(Math.toRadians(angle)) * distant);
        final float newY = (float) (getOrigin().getY() + StrictMath.sin(Math.toRadians(angle)) * distant);
        setOrigin(newX, newY);
    }
