...
public class EnemyA03 extends Enemy {
    ...
    public EnemyA03(@NotNull final Stage parent, final float x, final float y) {
        super(parent);
        position = new ImmutableVector2<>(x, y);
        direction = 0;
    }

    public EnemyA03(@NotNull final Stage parent, final Vector2<Float> position) {
        super(parent);
    ...
    private void fire() {
        ...
        for (int i = 0; i < fireCount; i++) {
            addSubTask(new ShotBall(this, (CIRCLE / fireCount) * i, ShotColor.RED));
        }
    }
